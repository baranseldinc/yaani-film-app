...
const firebaseConfig = {
  ...
  databaseURL: "https://yaani-film-app-default-rtdb.europe-west1.firebasedatabase.app"
};
...
export class FilmService {
  ...
  saveNewFilm(payload: any, callback: Function) {
    set(ref(this.db, `categories/${payload.categoryIndex}/list/${payload.nextIndex}`), {
  ...
  deleteFilm(categoryIndex: number, filmIndex: number, callback: Function) {
    remove(ref(this.db, `categories/${categoryIndex}/list/${filmIndex}`))
      .then(() => {
        callback();
      })
      .catch((error: any) => {
